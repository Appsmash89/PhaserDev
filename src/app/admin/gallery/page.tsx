...
function AssetRow({ setId, assetKey, urlField, label, accept, isImage, currentUrl, onUpdated, onDeleted }:
    { setId: string; assetKey: string; urlField: string; label: string; accept: string; isImage: boolean;
      currentUrl: string | null; onUpdated: (url: string) => void; onDeleted: () => void; }) {
    const inputRef = useRef<HTMLInputElement>(null);
    ...
    return (
        <div className="flex items-center gap-3 py-2.5 border-b border-white/5 last:border-0">
            <div className={`w-2 h-2 rounded-full flex-shrink-0 ${currentUrl ? 'bg-emerald-400' : 'bg-zinc-700'}`} />
            <div className="flex-1 min-w-0">
                <p className="text-xs font-bold text-zinc-300">{label}</p>
                {currentUrl
                    ? isImage
                        ? <img src={currentUrl} alt={label} className="mt-1.5 w-16 h-10 object-cover rounded-lg border border-white/10" />
                        : <p className="text-[10px] text-zinc-500 truncate mt-0.5">{currentUrl.split('/').pop()}</p>
                    : <p className="text-[10px] text-red-400/80 mt-0.5">Not uploaded</p>
                }
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
                <button onClick={() => inputRef.current?.click()} disabled={busy}
                        className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1.5
                                   bg-zinc-800 text-zinc-400 rounded-lg hover:bg-zinc-700
                                   hover:text-white transition-all disabled:opacity-40">
                    {busy ? '…' : currentUrl ? 'Replace' : 'Upload'}
                </button>
                {currentUrl && (
                    <button onClick={deleteAsset} disabled={busy}
                            className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1.5
                                        rounded-lg transition-all disabled:opacity-40
                                        ${confirmDelete ? 'bg-red-600 text-white' : 'bg-zinc-800 text-zinc-600 hover:text-red-400'}`}>
                            ...
                            <div className="flex items-center gap-4 p-4">
                                ...
                                <div className="flex flex-col gap-2 flex-shrink-0">
                                    <button onClick={() => setExpanded(expanded === set.id ? null : set.id)}
                                        ...
                                        {expanded === set.id ? 'Close' : 'Manage Assets'}
                                    </button>
                                    <button onClick={() => handleDeleteSet(set.id)}
                                        ...
                                        {deleting === set.id+'_confirm' ? '⚠ Confirm' : 'Delete Set'}
                                    </button>
                                </div>
                            </div>
                            {expanded === set.id && (
                                <div className="border-t border-white/5 px-4 py-2 bg-zinc-950/30">
                                    <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest py-2">
                                        Individual Assets
                                    </p>
                                    {ASSET_CONFIG.map(a => (
